...
from .database import DecodeError
from .database import Message
from .database import Node
from .database import Signal
from .database import SORT_SIGNALS_DEFAULT
from .database import sort_signals_by_start_bit
from .database import type_sort_signals
from .formats import AutosarDatabaseSpecifics
from .formats import arxml_load_string
from .formats import dbc_dump_string
from .formats import dbc_load_string
from .formats import InternalDatabase
from .formats import kcd_dump_string
from .formats import kcd_load_string
from .formats import sym_dump_string
from .formats import sym_load_string
from .formats.dbc import DbcSpecifics
from .tools import DecodeResultType
from .tools import EncodeInputType
from .tools import StringPathLike
...
class Database(object):
    ...
    def signals(self) -> List[Signal]:
        ...
        for message in self.messages:
            _signals.update(message.signals)
        return _signals
    ...
    def add_arxml_string(self, string: str) -> None:
        ...
        database = arxml_load_string(string, self._strict, sort_signals=self._sort_signals)
    ...
    def add_dbc_string(self, string: str) -> None:
        ...
        database = dbc_load_string(string, self._strict, sort_signals=self._sort_signals)
    ...
    def add_kcd_string(self, string: str) -> None:
        ...
        database = kcd_load_string(string, self._strict, sort_signals=self._sort_signals)
    ...
    def add_sym_string(self, string: str) -> None:
        ...
        database = sym_load_string(string, self._strict, sort_signals=self._sort_signals)
    ...
    def as_dbc_string(self, *, sort_signals: type_sort_signals = SORT_SIGNALS_DEFAULT) -> str:
        ...
        return dbc_dump_string(InternalDatabase(self._messages,
                                                self._nodes,
                                                self._buses,
                                                self._version,
                                                self._dbc),
    ...
    def as_kcd_string(self, *, sort_signals: type_sort_signals = SORT_SIGNALS_DEFAULT) -> str:
        ...
        return kcd_dump_string(InternalDatabase(self._messages,
                                                self._nodes,
                                                self._buses,
                                                self._version,
                                                self._dbc),
    ...
    def as_sym_string(self, *, sort_signals: type_sort_signals = SORT_SIGNALS_DEFAULT) -> str:
        ...
        return sym_dump_string(InternalDatabase(self._messages,
                                                self._nodes,
                                                self._buses,
                                                self._version,
                                                self._dbc),
    ...
    def get_message_by_name(self, name: str) -> Message:
        ...
        try:
            ...
            for _name in self._name_to_message.keys():
                if name.lower() in _name.lower():
                    logger.warning(f"The expected message name:{name} differs in case "
                                   f"from the message name:{_name} in the database {self.version}")
            raise ex
    ...
    def get_message_by_signal(self, sgn: Union[str, Signal]) -> Message:
        if isinstance(sgn, str):
            sgn = self.get_signal_by_name(sgn)
        for message in self.messages:
            if sgn in message.signals:
                return message
    ...
    def get_node_by_name(self, name: str) -> Node:
        ...
        for node in self._nodes:
            if node.name == name:
                ...
                logger.warning(f"The expected node name:{name} differs in case "
                               f"from the node name:{node.name} in the database {self.version}")

        raise KeyError(name)

    def get_bus_by_name(self, name: str) -> BusConfig:
        ...
        for bus in self._buses:
            if bus.name == name:
                return bus
            elif name.lower() == bus.name.lower():
                logger.warning(f"The expected bus name:{name} differs in case "
                               f"from the bus name:{bus.name} in the database {self.version}")
    ...
    def get_signal_by_name(self, name: str) -> Signal:
        for signal in self.signals:
            if name == signal.name:
                return signal
            elif name.lower() == signal.name.lower():
                logger.warning(f"The expected signal name:{name} differs in case "
                               f"from the signal name:{signal.name} in the database {self.version}")
