...
from typing import List, Optional, Union, Dict, TYPE_CHECKING, Set, Tuple, cast

from .signal import NamedSignalValue, Signal
from .signal_group import SignalGroup
from .utils import format_or, start_bit
...
from .errors import Error, EncodeError, DecodeError
from ..tools.typechecking import Comments, Codec, SignalDictType
from ..tools.typechecking import ContainerHeaderSpecType
from ..tools.typechecking import ContainerDecodeResultType
from ..tools.typechecking import  ContainerDecodeResultListType
from ..tools.typechecking import ContainerEncodeInputType
from ..tools.typechecking import EncodeInputType
from ..tools.typechecking import ContainerUnpackResultType
from ..tools.typechecking import ContainerUnpackListType
from ..tools.typechecking import DecodeResultType
if TYPE_CHECKING:
...
class Message(object):
    ...
    @property
    def ub_signals(self) -> List[Signal]:
        """A list of update bit signals in the message.
    ...
    @property
    def chks_signals(self) -> List[Signal]:
        """A list of checksum signals in the message.

        """ 
        
        new_signals = list()
        for sgn in self.signals:
            if sgn.name.endswith("Chks"):
                new_signals.append(sgn)
        return new_signals
    
    @property
    def cntr_signals(self) -> List[Signal]:
        """A list of counter signals in the message.

        """ 
        
        new_signals = list()
        for sgn in self.signals:
            if sgn.name.endswith("Cntr"):
                new_signals.append(sgn)
        return new_signals
    ...
    def get_contained_message_by_name(self, name: str) -> Optional['Message']:
    ...
    def get_signal_group_by_signal_name(self, name: str) -> SignalGroup:
        for signal_group in self.signal_groups:
            if name in signal_group.signal_names:
                return signal_group

    def get_special_signal_by_suffix(self, name: str, suffix: str) -> Union[Signal, None]:
        try:
            expected_signal = self.get_signal_by_name(f"{name}{suffix}")
        except:
            return None
        else:
